...
!========================================================================!
        ...
        implicit none
          ...
          subroutine field_init( I0_TWcm2, lambda_nm, ncyc )
            ...
            !tstart = -2.5d0 *  tau
            !tfin = 2.5d0 * tau 
            tstart = -(dble(ncycles+4))/2.d0*Tper 
          ...
          ! for future use 
            ...
            !Etx = ( E0*( (tau**2*w0 - 2.d0*t*Cos(2.d0*alphaQWP)) * Cos(t*w0) -& 
            !         ( 2.d0*t + tau**2*w0*Cos(2.d0*alphaQWP) ) * Sin(t*w0) ) )/&
            !         ( exp( (t/tau)**2 )*tau**2*w0 )
            !Ety = ( E0*Sin(2.d0*alphaQWP) * ( 2.d0*t*Cos(t*w0) + tau**2*w0*Sin( t*w0 ) ) )/&
            !         ( exp( (t/tau)**2 )*tau**2*w0 )
          ...
          ! multi-cycle vector potential (approximate) at time t, 2D
            ...
            !Atx = - A0*g_envelope(t) * (sin(w0*t + phiCEP) + cos(2.d0*alphaQWP)*cos(w0*t+ phiCEP))
            !Aty = - A0*g_envelope(t) * (                   - sin(2.d0*alphaQWP)*cos(w0*t+ phiCEP))
