...
!========================================================================!
        ...
        program main
          ...
          nkx = 200                     ! number of points in kx direction 
          nky = 200                        ! number of points in ky direction 
          ...
          sigp = 0.01                    ! local regularization factor: Gaussian width // eps = (f0*A0)**2 * exp(- (kx**2+ky**2) / sigma**2 )
          ! Note: sigp denotes the percent of the BZ to be covered. The actual width is given by: sigma = sigp * 4*pi/alat/3.0
          f0_  = 0.01                     ! regularization parameter entering eps = (f0 * A0)**2. Units: (Bohr)^(-1). Absolute value! 
          ! Integration portion of the BZ. Use 1.d0 for 1D, for 2D-significant speed-up and SNR improvement for bzcut = 0.25-0.5
          bzcut = 0.8d0
          theta = 0.0 * pi/180.d0      ! tilt angle between laser field and major ellipse axis
                    
          ! Lattice constants         
          aANG = 4.14! 4.138                   ! Angstroms; lattice constants of the layered triangular lattices 
          ...
          alphaQWP = 45.d0 * pi/180.d0 !pi/4.d0
...
!$omp end parallel
          ...
          call set_crystal(aANG, cANG, T2frac, f0_, f0_, 0.0d0, 1)
          ...
          do i = 1, ntpts
            ...
            if( mod(i, iprint) .eq.0 ) then
              ...
              !call print_group_velocity(iunit, t)
            endif
